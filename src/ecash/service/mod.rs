...
use super::config::{EcashConfig, LnConfig, MintConfig};

mod handlers;

pub async fn service(config_file_arg: String) -> anyhow::Result<tokio::task::JoinHandle<anyhow::Result<()>>> {
    ...
    let mint_service = MintService::new(ecash_config.clone()).await?;
    let ln_service = ClnService::new(ecash_config.clone()).await?;

    let mint_handle = mint_service.clone().serve(ln_service.clone().ln);
    let ln_handle = ln_service.serve(ecash_config.mint.last_pay_path, mint_service.clone().mint);
...
impl MintService {
    async fn new(ecash_config: EcashConfig) -> anyhow::Result<Self> {
    ...
    async fn serve(
        ...
        let state = MintState {
            ln,
            mint: Arc::new(tokio::sync::Mutex::new(self.mint)),
        };
        ...
        let ip = Ipv4Addr::from_str(&self.mint_config.listen_host)?;

        let port = self.mint_config.listen_port;
...
impl ClnService {
    ...
    async fn serve(
        &self,
        last_pay_path: String,
        mint: cdk::mint::Mint,
    ) -> anyhow::Result<tokio::task::JoinHandle<()>> {
        info!("Starting CLN service...");

        let ln = ln_rs::Ln {
            ln_processor: Arc::new(self.ln.clone()),
        };

        let ln_clone = ln.clone();
        let mint_clone = Arc::new(mint.clone());

        let last_pay_path = PathBuf::from_str(last_pay_path.as_str())?;
            ...
            loop {
                ...
                while let Some((invoice, pay_index)) = stream.next().await {
                    ...
                    if let Some(pay_index) = pay_index {
                        if let Err(err) = std::fs::write(last_pay_path.clone(), pay_index.to_be_bytes()) {
